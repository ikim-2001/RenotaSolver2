...
class Checker():
    def __init__(self):
        ...
        self.untouchable = copy.deepcopy(self.text)
        ...
        self.output = self.untouchable[-1].replace(" ", "")
        self.output = self.parse(self.output)
        self.output_left = self.output.split(self.comparator)[0]
        self.output_right = self.newlinesgone(self.output.split(self.comparator)[-1])
        self.student_output = self.solve(self.output_left, self.output_right)
